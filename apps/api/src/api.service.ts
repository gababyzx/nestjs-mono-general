...
import { EnvService } from 'libs/share/src/modules/env/env.service';
...
export class ApiService {
  ...
  getDefault() {
    const vars = this.envService.read();
    const workers = this.envService.currentWorker();
    const now = new Date();
    return {
      statusCode: 200,
      message: 'NestJS is da best!',
      name: vars.APP_NAME,
      server: process.env['HOST_HOSTNAME'] || process.env['HOSTNAME'] || os.hostname(),
